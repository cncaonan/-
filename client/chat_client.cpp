...
void* show_header(void* arg)
{
	...
	std::string msg = "welcom to chat system";
	int i = 1;
	//header 跑马灯实现，放置消息后重新绘框
	while(1)
	{
		//向框中间放置数据
...
{
	std::vector<std::string>::iterator iter = fd.begin();
	for(; iter!=fd.end(); ++iter)
	{
		if(user == *iter)
			return ;
	}
...
void* show_output_fd(void* arg)
{
	...
	while(1)
	{
		...
		for(;iter != fd.end(); ++iter)
		{
			...
			wrefresh(winp->friends_list);
			getmaxyx(winp->friends_list,fy, fx);
			if(j == fy - 1)
			{
				j=1;
...
void* show_input(void* arg)
{
	...
	while(1)
	{
		...
		wrefresh(winp->input);
		winp->get_str_from_win(winp->input,w.msg);
		//序列化之后发送
		w.str_to_serialize(out);
...
int main(int argc, char* argv[])
{
	...
	client.init_client();
	window win;
	net_window_t nw={&client, &win};//初始化
	qclient = &client;

	signal(2, quit);

	pthread_t theader;
	pthread_t toutput_fd;
	pthread_t tinput;
	pthread_create(&theader, NULL, show_header, &nw);
	pthread_create(&toutput_fd, NULL, show_output_fd, &nw);
	pthread_create(&tinput, NULL, show_input, &nw);

	pthread_join(theader, NULL);
	pthread_join(toutput_fd, NULL);
